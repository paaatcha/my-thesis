...
def cnfg():
    ...
    _csv_path_test = os.path.join(_base_path, "ISIC2019_parsed_test.csv")
    _imgs_folder_train = os.path.join(_base_path, "imgs")

    _use_meta_data = True
    _neurons_reducer_block = 0
    _comb_method = None # metanet, concat, or gcell
    _comb_config = None
    _batch_size = 30
    _epochs = 150
...
def main (_folder, _csv_path_train, _imgs_folder_train, _lr_init, _sched_factor, _sched_min_lr, _sched_patience,
          ...
          _best_metric, _neurons_reducer_block, _comb_method, _comb_config, _use_meta_data):
