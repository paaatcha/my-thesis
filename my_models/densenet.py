...
class MyDensenet (nn.Module):
    ...
    def __init__(self, densenet, num_class, neurons_reducer_block=256, freeze_conv=False, p_dropout=0.5,
                 comb_method=None, comb_config=None, n_feat_conv=1024):
        ...
        if comb_method is not None:
            ...
            if comb_method == 'gcell':
                ...
                self.comb = GCell(comb_config[0], comb_config[1])  # Normally (32, x)
                ...
                self.comb = MetaNet(comb_config[0], comb_config[1], comb_config[2]) # (n_meta, middle, 16)
    ...
    def forward(self, img, meta_data=None):
        ...
        x = self.features(img)
        x = nnF.relu(x, inplace=True)
        x = nnF.adaptive_avg_pool2d(x, (1, 1)).view(x.size(0), -1)
